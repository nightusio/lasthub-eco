...
@RequiredPermission(permission = "lasthub.eco.admin.turbo")
public class AdminSettingsEventTurboCommand extends BukkitCommand {
    ...
    @Inject
    public AdminSettingsEventTurboCommand(MessageConfig messageConfig, TimeConfig timeConfig, UserRepository userRepository) {
        super("turbodrop");
    ...
    public void content(@NonNull CommandSender commandSender, @NonNull String[] strings) {
        if (strings.length == 0 || strings[0].isEmpty()) {
            messageConfig.usage.send(commandSender, new MapBuilder<String, Object>()
                    .put("usage", "/austawienia turbodrop [gracz/all/off] [czas w sekundach] [mnożnik]").build());
            return;
        }

        if (strings[0].equalsIgnoreCase("off")) {
            if (timeConfig.globalTurboEvent) {
                timeConfig.globalTurboEventTime = 0;
        ...
        if (!strings[1].isEmpty() && !strings[2].isEmpty() || !strings[3].isEmpty()) {
            ...
            try {
                ...
                if (strings[0].equalsIgnoreCase("all")) {
                    timeConfig.globalTurboEvent = true;
                    timeConfig.globalTurboEventTime = time;
                    timeConfig.globalTurboEventStrength = multiplier;
                    timeConfig.save();

                    messageConfig.adminTurboEventGlobalSuccessfullyGiven.send(commandSender, new MapBuilder<String, Object>()
                    ...
                    targetUser.setEventTurbo(true);
                    targetUser.setEventTurboStrength(multiplier);
                    targetUser.setEventTurboTime(time);
                    targetUser.save();

                    this.messageConfig.adminTurboEventPlayerSuccessfullyGiven.send(commandSender, new MapBuilder<String, Object>()
                            .put("playerName", targetPlayer.getName())
                            .put("multiplier", multiplier)
                            .put("time", DateUtility.secondsToString(time))
                            .build());
                    this.messageConfig.eventTurboPlayerSuccessfullyGiven.send(targetPlayer, new MapBuilder<String, Object>()
